...
2. **캡처+편집** — 재생하며 스페이스바로 타이밍(cue) 기록(오디오 없으면 시작 버튼 기준 경과시간). 타임라인 바에 cue 마커와 재생 위치가 표시되고, 아래 가사조각 목록에서 각 cue의 start/duration/가사를 편집. 마커를 클릭하면 해당 목록 행으로 이동. 전체 가사를 붙여넣어 순차 분배 가능. 타임라인의 특정 위치부터 재캡처(덮어쓰기/삽입 모드)도 지원.
3. **Export** — 버전이 있는 JSON으로 다운로드/복사.
...
- ✅ UI 레이어 (setup / edit(캡처+편집 통합) / export 3단계) — 구현 완료.
- ✅ 타임라인 편집 — cue 마커·playhead, 마커 선택→목록 이동, 특정 위치 재캡처(덮어쓰기/삽입).
- ✅ 단일 HTML 빌드 (`dist/index.html`, JS/CSS 전부 인라인) 검증.
- ✅ 브라우저 E2E 스모크 테스트 — 통합 흐름(탭→마커 동기화→선택 연동→seek→덮어쓰기 재캡처) 정상.
